...
import br.com.twtm.estruturas.Pfs;
...
public class PfContaFR extends javax.swing.JFrame {
    ...
    public PfContaFR(Login novoLogin) {
        this.login = novoLogin;
        initComponents();
        dpf.preencherCampos(login);
        btDadosPessoais.setBackground(new java.awt.Color(51, 153, 255));
        lbBemVindo.setText("Bem-vindo, " + login.getEmail());
    }

    public PfContaFR(PessoaFisica pf) {
        this.ind = 1;
        this.pf = pf;
        initComponents();
        dpf.preencherCampos(pf);
        endp.preencherCampos(pf);
    ...
    public PfContaFR(Pessoa p, boolean estado) {
        initComponents();
        login = p.getLogin();
        this.pf = (PessoaFisica) p;
        dpf.preencherCampos(p, true);
    ...
    private void initComponents() {
        ...
        dpf = new br.com.twtm.gui.PfDadosCadastraisPL();
    ...
    private void lbDeixarParaDepoisMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_lbDeixarParaDepoisMouseClicked
        ...
        if (op == 0) {
            if (Pessoas.contem(pf)) {
                dpf.alterarPessoaFisica(pf, endp.criarEnd());
            } else {
                Pessoas.cadastrar(dpf.criarPessoaFisica(login, endp.criarEnd()));
            }
    ...
    private void btOkActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btOkActionPerformed
        ...
        if (ind == 0) {
            if (endp.vpCampos()) {
                ...
                PessoaFisica nova = (PessoaFisica) Logins.procurarLogin(login.getEmail(), login.getSenha());
                nova.perfil.setVer(true);
                PfTelaInicial t = new PfTelaInicial(nova);
    ...
    private br.com.twtm.gui.PfDadosCadastraisPL dpf;
